...
export class DocumentsComponent implements OnInit {
  ...
  constructor(
    ...
    this.overlayLoadingTemplate =
      '<span class="ag-overlay-loading-center">Por favor espere, estamos cargando los datos</span>';
    this.columnTypes = {
      numberColumn: {
        width: 130,
  ...
  async enviaracta() {
    ...
    const data = await this.restService
      .enviarcta(
        this.ciberseleccionado,
        this.info.tipo,
        this.info.curp,
        this.info.estado,
        nombrecompleto,
  ...
  giveMeDates() {
    this.database.getAllDates().subscribe(
      (data: any) => {
        this.dates = data;
      },
      (err: any) => {
  ...
  ChangeNewDate() {
    ...
    if (this.select.length > 1) {
      legend = ` ¿Estás seguro  de modificar la fecha a ${this.select.length.toString()} registros a ${
        this.newDateToChange
      }?`;
    } else {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        for (let i = 0; i < this.select.length; i++) {
          loader();
          this.restService
            .ChangeDate(this.select[i].id, this.newDateToChange)
            .subscribe(
              (data: any) => {
                closeAlert();
              },
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        for (let i = 0; i < this.select.length; i++) {
          ...
          this.restService.TransposeRegister(this.select[i].id, id).subscribe(
            (data: any) => {
              closeAlert();
            },
            (err: any) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        for (let i = 0; i < this.select.length; i++) {
          ...
          this.restService.DeleteRegister(this.select[i].id).subscribe(
            (data: any) => {
              closeAlert();
            },
            (err: any) => {
